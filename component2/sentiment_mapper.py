...
def main():
    ...
    # Emit after full book is read
    polarity = total_score / total_scored_words if total_scored_words > 0 else 0.0
    print(f"polarity_score\t{polarity:.4f}")
    print(f"positive_count\t{float(positive_count):.1f}")
    print(f"negative_count\t{float(negative_count):.1f}")
    print(f"total_scored_words\t{float(total_scored_words):.1f}")
